 ...
 *  JDBC를 이용한 데이터베이스 처리 순서
 *  
 *  순서 : JDBC 드라이버 로딩 => 해당 DB에 접속 => 질의(SQL명령을 수행한다.)
 ...
 */
	public static void main(String[] args) {
		...
		Statement stmt = null;
		ResultSet rs = null; //쿼리문이 select일 때 필요함
		...
		try {
			...
			//4. SQL문을 Statement 객체를 이용하여 실행하고 실행 결과를 ResultSet객체에 저장한다.
			...
			//rs.next() => ResultSet 객체의 데이터를 가리키는 포인터를 다음 레코드로 이동시키고 그곳에 자료가 있으면 true, 없으면 false를 반환한다.
			
			while(rs.next()) {
				//컬럼의 자료를 가져오는 방법
				//방법1) rs.get자료형 이름("컬럼명")
				//방법2) rs.get자료형 이름(컬럼번호) => 컬럼번호는 1번부터 시작/
