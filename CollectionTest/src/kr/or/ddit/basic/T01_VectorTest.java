...
public class T01_VectorTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("set 명령 후 v1 ==> " + v1.toString());
		System.out.println("원래의 데이터 : " + temp);
		
		//remove(index) ==> 벡터의 index번쨰 자료를 삭제한다.
		...
		v1.remove(0);
		System.out.println("삭제 후 : " + v1.toString());
		System.out.println();
		...
		/*
		 *  - 제너릭 타입 (generic type)
		 *    => Collection 객체를 선언할 때 <> 안에 그 Collection이 저장될 데이터 타입을 정해주는 것을 말한다.
		 ...
		 *    => 제너릭 타입으로 선언하게 되면 데이터를 꺼내올 떄 별도의 형변환이 필요없다.
		 */
		...
		System.out.println("삭제되기 전 v2 => " + v2.toString());
		
		//removeAll(collection객체) ==> 벡터의 데이터 중에서 'Collection객체'가 가지고 있는 데이터를 모두 삭제한다.
		v2.removeAll(v4);
		System.out.println("삭제된 후 v2 => " + v2.toString());
		System.out.println("------------------------------------");
		...
		/*
		 *  - 향상된 for문 형식
		 *  for(자료형명 변수명 : 배열 변수나 Collection계열 변수){
		 *  	처리할 내용들;
		 *  	...
		 *  }
		...
		//만약 제너릭을 사용하지 않는 Collection를 향상된 for문으로 처리할 경우에는 Object형으로 간주한다.
		for(Object obj : v1) {
		...
		System.out.println("==========After v.emsureCapacity(5)==========");
		print(v);
