 ...
 * 3.이 인스턴스를 각각 쓰레드에 넘겨준다.
 * 4.각각 쓰레드는 이 인스턴스의 참조값을 저장한 변수를 이용하여 공통 데이터를 사용한다.
 * 
...
public class T14_ThreadShareDataTest {
	public static void main(String[] args) {
		...
		PrintThread ppt = new PrintThread(sd);
...
class CalcPIThread extends Thread{
	...
	public void run() {
		...
		for(int i = 1; i <= 1500000000; i+=2) {
			if(((i/2)%2) == 0){//2로 나눈 몫이 짝수이면(2로 나누었을 때 나머지 0) +
				sum+= (1.0 + i);
			}else {
				sum -= (1.0/i);
			}
		}
		sd.result = sum * 4; //계산된 원주율을 공동객체의 멤버변수에 저장
...
class PrintThread extends Thread{
	...
	public void run() {
		while(true) {
			if(sd.isOk) {//원주율 계산이 완료될때 까지 기다린다.
				break;
			}
		}
		System.out.println();
