 ...
 * 회원정보 VO
 */
class Member implements Serializable{ //자바는 Serializable 인터페이스를 구현한 클래스만 직렬화 할 수 있도록 제한하고 있음
									  //구현 안하면 직렬화 작업시 java.io.NotSerializableException 발생!
	//transient => 직렬화가 되지 않을 멤버변수에 지정한다.
	//			   (* static 필드도 직렬화가 되지 않는다.)
	//			      직렬화가 되지 않는 멤버 변수는 기본 값으로 저장한다.
	//			   (참조변수 : null, 숫자형 변수 : 0)
